...
class GenerateGraphDialog:
    ...
    def __init__(self, parent, on_generate):
        ...
        ttk.Label(self.window, text="Количество вершин").grid(row=0, column=0, padx=10, pady=8, sticky="w")
        self.vertices = ttk.Entry(self.window)
        self.vertices.insert(0, "20")
        self.vertices.grid(row=0, column=1)
        
        ttk.Label(self.window, text="Вероятность появления ребра").grid(row=1, column=0, padx=10, pady=8, sticky="w")
        self.probability = ttk.Entry(self.window)
        self.probability.insert(0, "0.3")
        self.probability.grid(row=1, column=1)
        
        ttk.Label(self.window, text="Минимальный вес").grid(row=2, column=0, padx=10, pady=8, sticky="w")
        self.min_weight = ttk.Entry(self.window)
        self.min_weight.insert(0, "1")
        self.min_weight.grid(row=2, column=1)
        
        ttk.Label(self.window, text="Максимальный вес").grid(row=3, column=0, padx=10, pady=8, sticky="w")
        ...
        ttk.Button(btn_frame, text="Сгенерировать", command=self._create).pack(side="left", padx=10)
        ttk.Button(btn_frame, text="Отмена", command=self.window.destroy).pack(side="left", padx=10)
