...
class ManualInputDialog:
    ...
    def __init__(self, parent, on_save):
        ...
        ttk.Label(
            self.window, text=instruction, justify="left", font=("Consolas", 9)
        ...
        ttk.Button(btn_frame, text="Применить", command=self._parse_and_save).pack(
            side="left", padx=20
        )
    ...
    def _parse_and_save(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Ошибка парсинга", f"Некорректный формат данных!\n{str(e)}"
            )
            return

        if not edges:
            messagebox.showwarning(
                "Предупреждение", "Не удалось распознать ни одного ребра."
            )
            return
