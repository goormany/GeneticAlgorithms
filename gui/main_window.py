...
class GA_GUI:
    ...
    def _create_layout(self):
        ...
        self.individual_combo = ttk.Combobox(
            selector_frame, textvariable=self.individual_var, state="readonly", width=40
        )
    ...
    def _on_manual_input(self, edges):
        ...
        self._update_after_graph_load(
            f"Граф введен вручную: вершин={num_vertices}, ребер={len(edges)}"
        )
    ...
    def load_in_ga(self):
        ...
        self.ga = GeneticAlgorithmMST(self.graph_manager.get_graph(), **params)
    ...
    def _populate_individual_selector(self):
        ...
        for sol in self.individual_solutions:
            ...
            labels.append(
                f"#{sol['rank']} (вес: {weight_str}) | код: {'-'.join(map(str, sol['prufer_code']))}"
            )
    ...
    def _on_individual_selected(self, event=None):
        ...
        try:
            ...
            if 0 <= idx < len(self.individual_solutions):
                ...
                self.individual_info_label.configure(text=f"вес: {sol['weight']:.2f}")
        ...
        if stats:
            ...
            if best_solution:
                ...
                if is_valid and mst_weight < self.best_mst_weight:
                    ...
                    self.log.insert(
                        "end",
                        f"Найдено новое решение: Вес: {mst_weight}, поколение: {generation}\n",
                    )
                    self.log.see("end")
    ...
    def prev_step(self):
        ...
        self.log.insert(
            "end",
            f"Откат с поколения {old_generation} к поколению {self.ga.generation}\n",
        )
        self.log.see("end")
        ...
        for gen_idx in range(history_len_before, len(self.ga.best_fitness_history)):
            ...
            if is_valid:
                ...
                if weight < self.best_mst_weight:
                    ...
                    self.log.insert(
                        "end",
                        f"Найдено новое решение: Вес: {weight:.2f}, поколение: {gen_idx}\n",
                    )
                    self.log.see("end")
