...
class ControlButtons:
    ...
    def __init__(self, parent, on_generate=None, on_load=None, on_manual=None,
                 on_run=None, on_reset=None, on_step_back=None, 
                 on_step_forward=None, on_play=None, on_skip=None):
        
        # Кнопки управления графом
        buttons = ttk.Frame(parent)
        buttons.pack(fill="x", pady=5)
        
        self.btn_generate = ttk.Button(buttons, text="Сгенерировать граф", command=on_generate)
        ...
        self.btn_run = ttk.Button(algo_buttons, text="Запустить алгоритм", command=on_run)
        ...
        self.btn_back = ttk.Button(step_controls, text="Шаг назад", 
                                   command=on_step_back, state="disabled")
        self.btn_back.pack(side="left", padx=2)
        
        self.btn_play = ttk.Button(step_controls, text="▶️ Авто-запуск",
                                   command=on_play, state="disabled")
        self.btn_play.pack(side="left", padx=2)
        
        self.btn_next = ttk.Button(step_controls, text="Шаг вперед",
                                   command=on_step_forward, state="disabled")
        self.btn_next.pack(side="left", padx=2)
        
        self.btn_skip = ttk.Button(step_controls, text="⏯️ Пропустить",
                                   command=on_skip, state="disabled")
