...
class GAStateStep(BaseModel):
    ...
    population: list[list[int]] = Field(
        description="Список хромосом(кодов Прюфера)", default_factory=list
    )
    ...
    best_edges: list[tuple[int, int]] = Field(
        description="Лучие ребра из популяции", default_factory=list
    )
...
class GAHistory(BaseModel):
    ...
    best_fitness_history: list[float] = Field(
        description="История лучшего фитнеса", default_factory=list
    )
    avg_fitness_history: list[float] = Field(
        description="История среднего фитнеса", default_factory=list
    )
    worst_fitness_history: list[float] = Field(
        description="История худшего фитнеса", default_factory=list
    )
    steps: list[GAStateStep] = Field(
        description="Шаги алгоритма (состояний)", default_factory=list
    )
