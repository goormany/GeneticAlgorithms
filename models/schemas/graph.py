...
class GraphData(BaseModel):
    ...
    def validate_matrix(self):
        for u in range(self.num_vercites):
            for v in range(self.num_vercites):
                if (
                    u == v
                    and self.matrix[u][v] == self.matrix[v][u]
                    and self.matrix[u][v] != 0
                ):
    ...
    def from_edges_list(cls, n: int, edges: list[tuple[int, int, float]]):
        ...
        return cls(num_vercites=n, matrix=matrix)

    def to_edges_list(self) -> list[tuple[int, int, float]]:
        return [
            (u, v, self.matrix[u - 1][v - 1])
            for u in range(1, self.num_vercites + 1)
            for v in range(1, self.num_vercites + 1)
            if self.matrix[u - 1][v - 1] > 0
        ]
